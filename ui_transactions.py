...
class PopupOperation(ctk.CTkToplevel):
    ...
    def _build(self, titre):
        ...
        if self.mode == "transfert":
            self.e_dest = row("ID compte destinataire", "Ex : 2")
    ...
    def _submit(self):
        try:
            ...
            if   self.mode == "depot":
                ...
                transferer(self.compte_id, int(self.e_dest.get()), montant, desc, cat)
...
class PageTransactions(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        if len(self.comptes) > 1:
            ...
            self.v_compte = tk.StringVar(value=noms[0])
            ctk.CTkComboBox(dr, variable=self.v_compte, values=noms,
                            width=220, height=34, fg_color=C["carte"],
                            border_color=C["bordure"], button_color=C["bleu"],
                            dropdown_fg_color=C["carte"],
                            command=self._changer_compte).pack(pady=(0, 6))
    ...
    def rafraichir(self):
        ...
        if self.comptes and not self.compte_actif:
